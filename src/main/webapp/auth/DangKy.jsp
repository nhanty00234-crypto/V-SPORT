...
<body class="min-h-screen h-screen w-screen overflow-hidden bg-[#f8fafc]">
    ...
    <script>
        ...
        // Bind form submit spinner & validation
        function bindFormSubmit() {
            ...
            if (mainRegisterForm && mainRegisterBtn) {
                mainRegisterForm.onsubmit = function(event) {
                    const username = mainRegisterForm.username.value.trim();
                    const email = mainRegisterForm.email.value.trim();
                    const phone = mainRegisterForm.phone.value.trim();
                    const password = document.getElementById('reg-pass').value;
                    const confirmPassword = document.getElementById('reg-confirm-pass').value;

                    // Remove existing client-side error banner if any
                    const existingBanner = document.getElementById('client-error-banner');
                    if (existingBanner) existingBanner.remove();

                    function showError(msg) {
                        const bannerHtml = `
                            <div id="client-error-banner" class="mb-5 p-3.5 bg-red-50 text-red-600 rounded-xl text-xs font-semibold border border-red-100 flex items-center gap-3 shadow-sm">
                                <span class="material-symbols-outlined text-[18px]">error</span>
                                <span>${msg}</span>
                            </div>
                        `;
                        mainRegisterForm.insertAdjacentHTML('beforebegin', bannerHtml);
                        const scrollPanel = document.querySelector('.register-form-panel');
