...
<c:set var="profileFallbackAvatarUrl" value="https://ui-avatars.com/api/?name=${profileAvatarName}&background=7c3aed&color=fff&bold=true" />
    ...
    <div class="relative px-5 py-4 border-b border-zinc-100">
      ...
      <div class="flex items-start gap-3.5">
        <div class="relative shrink-0">
          <img class="profile-avatar-img w-[52px] h-[52px] rounded-full object-cover ring-2 ring-purple-600/30" src="${profileAvatarUrl}" alt="Avatar">
  ...
  <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-[520px] max-h-[90vh] overflow-y-auto z-10 border border-zinc-200 modal-animate-scale">
    <form onsubmit="return false;" autocomplete="off" class="flex flex-col">
      ...
      <div class="flex flex-col items-center gap-3 pt-6 pb-4 border-b border-zinc-100">
        <div class="relative">
          <img id="editAvatarPreview" class="profile-avatar-img w-20 h-20 rounded-full object-cover ring-2 ring-purple-600/30" src="${profileAvatarUrl}" alt="Avatar">
          <label for="avatarUpload" class="absolute bottom-0 right-0 w-7 h-7 rounded-full bg-purple-700 text-white flex items-center justify-center cursor-pointer shadow hover:opacity-90 transition-opacity"><span class="material-symbols-outlined text-[14px]">photo_camera</span></label>
      ...
      <div class="px-6 pb-5 flex gap-3 justify-end">
        <button type="button" onclick="document.getElementById('editProfileModal').classList.add('hidden')" class="h-10 px-5 rounded-lg border border-zinc-200 text-sm font-semibold text-zinc-650 hover:bg-zinc-50 transition-colors">Hủy</button>
        <button type="button" onclick="saveProfileChanges()" class="h-10 px-5 rounded-lg bg-purple-700 text-white text-sm font-semibold hover:bg-purple-800 transition-colors active:scale-95">Lưu thay đổi</button>
      </div>
    </form>
  </div>
</div>
  ...
  <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-[420px] z-10 border border-zinc-200 modal-animate-scale">
    <div class="px-6 py-4 border-b border-zinc-200 flex items-center justify-between">
      <div class="flex items-center gap-2.5">
        <span class="material-symbols-outlined text-[20px] text-purple-700">mark_email_read</span>
    ...
    <div class="px-6 py-5">
      ...
      <div class="flex gap-2 justify-center" id="profileOtpBoxes">
        <input type="text" inputmode="numeric" maxlength="1" class="otp-box-mgr w-11 text-center text-xl font-black border-2 border-zinc-200 rounded-xl focus:border-purple-700 focus:outline-none transition-colors bg-zinc-50 focus:bg-white" style="height:52px;" data-idx="0">
        <input type="text" inputmode="numeric" maxlength="1" class="otp-box-mgr w-11 text-center text-xl font-black border-2 border-zinc-200 rounded-xl focus:border-purple-700 focus:outline-none transition-colors bg-zinc-50 focus:bg-white" style="height:52px;" data-idx="1">
        <input type="text" inputmode="numeric" maxlength="1" class="otp-box-mgr w-11 text-center text-xl font-black border-2 border-zinc-200 rounded-xl focus:border-purple-700 focus:outline-none transition-colors bg-zinc-50 focus:bg-white" style="height:52px;" data-idx="2">
        <span class="flex items-center text-zinc-300 font-bold text-lg select-none">—</span>
        <input type="text" inputmode="numeric" maxlength="1" class="otp-box-mgr w-11 text-center text-xl font-black border-2 border-zinc-200 rounded-xl focus:border-purple-700 focus:outline-none transition-colors bg-zinc-50 focus:bg-white" style="height:52px;" data-idx="3">
        <input type="text" inputmode="numeric" maxlength="1" class="otp-box-mgr w-11 text-center text-xl font-black border-2 border-zinc-200 rounded-xl focus:border-purple-700 focus:outline-none transition-colors bg-zinc-50 focus:bg-white" style="height:52px;" data-idx="4">
        <input type="text" inputmode="numeric" maxlength="1" class="otp-box-mgr w-11 text-center text-xl font-black border-2 border-zinc-200 rounded-xl focus:border-purple-700 focus:outline-none transition-colors bg-zinc-50 focus:bg-white" style="height:52px;" data-idx="5">
      </div>
    ...
    <div class="px-6 pb-5 flex gap-3 justify-end">
      ...
      <button type="button" onclick="verifyProfileEmailOtp()" class="h-10 px-5 rounded-lg bg-purple-700 text-white text-sm font-semibold hover:bg-purple-800 transition-colors active:scale-95">Xác thực</button>
    </div>
  </div>
</div>
  ...
  <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-[400px] z-10 border border-zinc-200 modal-animate-scale">
    <form onsubmit="return false;" autocomplete="off" class="flex flex-col">
      ...
      <div class="px-6 pb-5 flex gap-3 justify-end">
        ...
        <button type="button" onclick="savePassword()" class="h-10 px-5 rounded-lg bg-purple-700 text-white text-sm font-semibold hover:bg-purple-800 transition-colors active:scale-95 font-sans">Đổi mật khẩu</button>
      </div>
    </form>
  </div>
</div>
...
<script>
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    const elementsToMove = ['editProfileModal', 'profileEmailOtpModal', 'changePwModal', 'successToast'];
      ...
      .then(data => {
        ...
        if (data.success) {
          if (data.requiresOtp) {
            ...
            setTimeout(() => { const b = document.querySelector('#profileOtpBoxes .otp-box-mgr'); if(b) b.focus(); }, 80);
  ...
  function _getMgrOtp() {
    return Array.from(document.querySelectorAll('#profileOtpBoxes .otp-box-mgr')).map(i => i.value).join('');
  }

  function _clearMgrOtpBoxes() {
    document.querySelectorAll('#profileOtpBoxes .otp-box-mgr').forEach(b => {
      b.value = '';
      b.classList.remove('border-red-400', 'border-purple-700');
      b.classList.add('border-zinc-200');
    });
  }

  function _initMgrOtpBoxes() {
    const boxes = document.querySelectorAll('#profileOtpBoxes .otp-box-mgr');
  ...
  document.addEventListener('DOMContentLoaded', _initMgrOtpBoxes);

  function verifyProfileEmailOtp() {
    const otp = _getMgrOtp();
    const err = document.getElementById('profileEmailOtpError');
    if (!/^\d{6}$/.test(otp)) {
      err.textContent = 'Vui lòng nhập đủ 6 chữ số.';
      err.classList.remove('hidden');
      document.querySelectorAll('#profileOtpBoxes .otp-box-mgr').forEach(b => b.classList.add('border-red-400'));
      return;
    }
