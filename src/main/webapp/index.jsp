...
<head>
    ...
    <style>
        ...
        /* ============================ Top header (navy band) ========================= */
        .vs-topband {
            width: 100%; background: var(--vs-primary-900, #0B2545); position: relative; overflow: hidden;
            padding: 9px 10px 9px;
        }
        @media (min-width: 768px) { .vs-topband { padding: 9px 12px 9px; } }
        /* Abstract organic shapes (CSS only, no external asset) — echoes the
           patterned sports band of the reference without copying any artwork. */
        .vs-topband::before {
            content: ""; position: absolute; inset: 0; pointer-events: none;
            background:
                radial-gradient(ellipse 420px 200px at 22% 130%, rgba(24, 200, 232, 0.32), transparent 62%),
                radial-gradient(ellipse 520px 260px at 48% -40%, rgba(22, 119, 210, 0.42), transparent 60%),
                radial-gradient(ellipse 380px 240px at 72% 140%, rgba(7, 26, 47, 0.70), transparent 65%),
                radial-gradient(ellipse 300px 190px at 90% -30%, rgba(24, 200, 232, 0.24), transparent 60%),
                radial-gradient(ellipse 260px 170px at 99% 110%, rgba(7, 26, 47, 0.60), transparent 68%),
        ...
        .vs-auth-btn-login { background: #fff; color: var(--vs-primary-900, #0B2545); border: 1px solid #fff; }
        .vs-auth-btn-login:hover { background: var(--vs-cyan-50, #F0FCFE); }
        ...
        .vs-utilitybar {
            width: 100%; background: #fff; border: 1px solid var(--vs-border, #DCE5EF); border-radius: 8px;
        ...
        .vs-ub-filter {
            ...
            color: var(--vs-cyan-600, #08A9CC); transition: background-color .15s ease;
        }
        .vs-ub-filter .material-symbols-outlined { font-size: 21px; }
        .vs-ub-filter:hover { background: var(--vs-cyan-50, #F0FCFE); }
        .vs-ub-action {
        ...
        .vs-ub-action:hover { background: var(--vs-cyan-50, #F0FCFE); }
        .vs-ub-action .material-symbols-outlined { font-size: 20px; color: var(--vs-primary-600, #1677D2); flex-shrink: 0; }
        ...
        /* Booking-choice modal, facility bottom-sheet, page-transition loader and
           skeleton styles now live in customer/common/facility-interactions.jsp
           (V-SPORT Navy/Blue/Cyan/Orange tokens) — single source of truth. */
...
<body class="bg-surface text-on-surface antialiased overflow-x-hidden">
    ...
    <!-- Shared facility interactions: home toast, "Chọn hình thức đặt" modal,
         full-screen page-transition loader, and facility-detail bottom sheet.
         Single source of truth (also used by TimKiem.jsp) — routes "Đặt sân
         trực tiếp" to /customer/dat-lich-truc-quan?coSoId= with press-effect. -->
    ...
    <script>
        ...
        function shareFacility(name, id) {
            ...
            if (navigator.clipboard) {
                const url = window.location.origin + "<%= ctx %>/customer/dat-lich-truc-quan?coSoId=" + id;
