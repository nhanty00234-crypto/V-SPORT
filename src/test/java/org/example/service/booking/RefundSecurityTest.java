...
class RefundSecurityTest {
    ...
    @Test
    @DisplayName("4. Early cancellation vs Late cancellation thresholds are correctly calculated")
    void testCancellationTypeDecision() {
        var now = java.time.LocalDateTime.of(2026, 7, 28, 10, 0);
        var earlyBooking = java.time.LocalDateTime.of(2026, 7, 28, 20, 0); // 10 hrs away (> 6 hrs)
        var lateBooking = java.time.LocalDateTime.of(2026, 7, 28, 12, 0);  // 2 hrs away (< 6 hrs)

        assertEquals(CancelDecision.CancelType.EARLY_CANCEL, CancelDecision.decide(now, earlyBooking, 6));
        assertEquals(CancelDecision.CancelType.LATE_CANCEL, CancelDecision.decide(now, lateBooking, 6));
    }
}
