...
1. **`/admin/quan-ly-san` (`QuanLySanServlet`) is unreachable in production.** `FilterQuyenAdmin.java:32-35` unconditionally 403s any path containing `/quan-ly-san` or `/QuanLySan.jsp` under `/admin/*`, even for Admin. Real "manage courts" traffic goes through `/manager/quan-ly-san` instead (`QuanLySanManagerServlet`, confirmed small/bounded dataset, no pagination needed per audit). This is a pre-existing access-control bug unrelated to pagination — report it, do not fix it here.
