import { getAdminBookings } from '@/lib/api/admin-pages'

export const metadata = { title: 'Lịch đặt sân | V-SPORT Admin' }
...
export default async function AdminLichDatSanPage() {
  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          { label: 'Tổng lịch đặt', value: bookings.length, color: 'text-blue-700' },
      ...
      <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden">
        <div className="px-5 py-4 border-b border-slate-100">
          <h3 className="font-bold text-slate-800">Lịch đặt sân toàn hệ thống (200 gần nhất)</h3>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {bookings.length === 0 ? (
                <tr><td colSpan={7} className="px-4 py-8 text-center text-slate-400">Không có lịch đặt sân</td></tr>
              ) : bookings.map(b => (
                <tr key={b.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-4 py-3 text-slate-500">{b.tenSan || `Sân #${b.sanId}`}</td>
                  <td className="px-4 py-3 text-slate-600">{b.ngayDat}</td>
                  <td className="px-4 py-3 text-xs text-slate-400">{b.gioBatDau}–{b.gioKetThuc}</td>
                  <td className="px-4 py-3 text-right font-bold text-slate-800">{b.tongTien.toLocaleString('vi-VN')}đ</td>
                  <td className="px-4 py-3">
